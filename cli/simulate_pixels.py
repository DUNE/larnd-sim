...
def run_simulation(input_filename,
    ...
    if sim.IS_SPILL_SIM:
        ...
        localSpillIDs = localSpillIDs = tracks[sim.EVENT_SEPARATOR] #(events start at 0, not 1000)# - (tracks[sim.EVENT_SEPARATOR] // sim.MAX_EVENTS_PER_FILE) * sim.MAX_EVENTS_PER_FILE
    ...
    num_evids = (tracks[sim.EVENT_SEPARATOR].max()) + 1 #(events start at 0, not 1000)# % sim.MAX_EVENTS_PER_FILE) + 1
    if sim.IS_SPILL_SIM:
    ...
    def save_results(event_times, is_first_batch, results):
        ...
        uniq_event_times = cp.asnumpy(event_times[uniq_events]) #(events start at 0, not 1000)# % sim.MAX_EVENTS_PER_FILE])
        if light.LIGHT_SIMULATED:
