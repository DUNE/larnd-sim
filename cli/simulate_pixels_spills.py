...
from numba.cuda import device_array, to_device, synchronize
...
def run_simulation(input_filename,
    ...
    for batch_mask in tqdm(batching.TPCBatcher(tracks, EVENT_SEPARATOR, tpc_batch_size=EVENT_BATCH_SIZE, tpc_borders=detector.TPC_BORDERS),
        ...
        if len(track_subset) == 0:
            continue
        ievd = int(track_subset[0][EVENT_SEPARATOR])
        ...
        for itrk in tqdm(range(0, evt_tracks.shape[0], BATCH_SIZE),
            ...
            event_ids = selected_tracks[EVENT_SEPARATOR]
            ...
            TPB = 128
            BPG = max(ceil(pixels_signals.shape[0] / TPB),1)
            ...
            pixel_thresholds = cp.full(unique_pix.shape[0],7000.,float)
