...
def export_to_hdf5(event_id_list,
    ...
    if packets:
        ...
        if packets_mc.shape[1] >= TRACK_ASSOCIATION_COUNT:
            packets_mc_ds['track_ids'] = packets_mc   [:,:TRACK_ASSOCIATION_COUNT]
            packets_mc_ds['fraction' ] = packets_frac [:,:TRACK_ASSOCIATION_COUNT]
        else:
            num_to_pad = TRACK_ASSOCIATION_COUNT - packets_mc.shape[1]
            packets_mc_ds['track_ids'] = np.pad(packets_mc,
                pad_width=((0,0),(0,num_to_pad)),
                mode='constant',
                constant_values=-1)
            packets_mc_ds['fraction' ] = np.pad(packets_frac,
                pad_width=((0,0),(0,num_to_pad)),
                mode='constant',
                constant_values=0.)
