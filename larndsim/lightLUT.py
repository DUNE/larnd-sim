...
def get_voxel(pos, itpc):
    ...
    if is_odd:
        i = int((pos[0] - tpc_borders[0][0])/(tpc_borders[0][1] - tpc_borders[0][0])*consts.lut_vox_div[0]) 
    else:
        i = int((tpc_borders[0][1] - pos[0])/(tpc_borders[0][1] - tpc_borders[0][0])*consts.lut_vox_div[0]) 
    j = int((pos[1] - tpc_borders[1][0])/(tpc_borders[1][1] - tpc_borders[1][0])*consts.lut_vox_div[1]) 
    k = int((pos[2] - tpc_borders[2][0])/(tpc_borders[2][1] - tpc_borders[2][0])*consts.lut_vox_div[2]) 
