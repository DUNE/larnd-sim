...
def export_light_wvfm_to_hdf5(event_id, waveforms, output_filename, waveforms_true_track_id, waveforms_true_photons, i_mod=-1):
    ...
    with h5py.File(output_filename, 'a') as f:
        ...
        if waveforms_true_track_id.size > 0:
            truth_dtype = np.dtype([('track_id', 'i8', (waveforms_true_track_id.shape[-1],)), ('tick', 'i4', (waveforms_true_track_id.shape[-1],)), ('pe_current', 'f8', (waveforms_true_track_id.shape[-1],))])
            truth_data = np.zeros(waveforms_true_track_id.shape[:-2], dtype=truth_dtype)
